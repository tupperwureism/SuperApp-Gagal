-- Batch 3.B.1: preserve signed webhook replay after downstream progression and
-- minimize direct service-role event-table privileges. Provider initiation
...
BEGIN
  ...
  -- These records define immutable event-to-settlement binding. Their mutable
  -- workflow states are deliberately not part of processed-event replay.
  SELECT *
  INTO v_case
  ...
  IF NOT FOUND OR v_case.order_id <> p_order_id THEN
  ...
  IF NOT FOUND OR v_escrow.corporate_case_id IS DISTINCT FROM p_case_id THEN
  ...
  SELECT pg_catalog.count(*),
         pg_catalog.count(*) FILTER (WHERE milestone.funded_at IS NOT NULL)
  ...
  IF v_was_processed THEN
    IF v_event.processed_at IS NULL
       OR v_funded_milestones = 0 THEN
      RAISE EXCEPTION 'CORPORATE_PAYMENT_WEBHOOK_EVENT_STATE_CONFLICT';
    END IF;

    -- The receipt is the original canonical settlement result. Current case,
    -- order, escrow, or milestone workflow states may legitimately be later.
    RETURN QUERY
    SELECT
      v_event.event_id,
...
) IS 'Server-only signed corporate payment callback boundary. Initial processing atomically advances escrow, case, order, milestones, and provider event. Identical processed replay validates immutable identity and persistent binding, then returns the original receipt without depending on mutable downstream workflow state. Inbound funding evidence uses provider_webhook_events plus existing compliance/WORM evidence; no dedicated inbound funding ledger exists.';
