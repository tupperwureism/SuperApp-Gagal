...
export const PortalCardItem: React.FC<PortalCardItemProps> = ({
  ...
  return (
    <Card className="flex flex-col justify-between gap-8 p-8 xl:p-12 rounded-3xl border border-white/15 bg-slate-900/90 backdrop-blur-2xl shadow-2xl min-h-[480px] lg:min-h-[520px] h-full relative overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:border-white/40 hover:shadow-[0_25px_60px_rgba(0,0,0,0.7)] group">
      ...
      <CardHeader className="p-0 pt-3 flex flex-col items-start gap-4">
        <Badge
          variant="outline"
          className={`px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 shadow-sm ${badgeClass}`}
        >
          {BadgeIcon && <BadgeIcon className="w-3.5 h-3.5" />}
          <span>{badge}</span>
        </Badge>

        <h3 className="text-2xl md:text-3xl xl:text-4xl font-black text-white font-heading tracking-tight">
          {title}
        </h3>
      </CardHeader>
      ...
      <CardFooter className="p-0 mt-auto pt-4">
        <Button
          asChild
          size="lg"
          className={`w-full h-14 rounded-2xl font-extrabold text-sm md:text-base gap-3 shadow-xl transition-all ${btnClass}`}
        >
